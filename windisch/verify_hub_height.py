...
data["Minimum hub height"] = pd.to_numeric(data["Minimum hub height"].replace("#ND", np.nan), errors="coerce")
data["Maximum hub height"] = pd.to_numeric(data["Maximum hub height"].replace("#ND", np.nan), errors="coerce")
data.dropna(subset=["Rated power", "Minimum hub height", "Maximum hub height"], inplace=True)

# Calculate average hub height if both min and max are available
data["Observed hub height"] = data[["Minimum hub height", "Maximum hub height"]].mean(axis=1)
...
def analyze_hub_height(data_subset, turbine_type):
    ...
    coeffs, _ = curve_fit(hub_height_model, rated_power, observed_hub_height, p0=initial_guess)
    ...
    plt.scatter(rated_power, observed_hub_height, label="Observed", color="blue", alpha=0.6)
    plt.plot(
        np.sort(rated_power),
    ...
    print(f"Coefficients: coeff_a={coeffs[0]:.2f}, coeff_b={coeffs[1]:.2f}, coeff_c={coeffs[2]:.2f}")
