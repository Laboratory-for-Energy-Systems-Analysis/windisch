...
for index, row in dataframe.iloc[1:].iterrows():
    ...
    cut_in_col = (
        row.iloc[:71] != 0
    ).idxmax()  # Finding the first column index where value isn't zero
    if (
        cut_in_col in dataframe.columns
    ):  # Ensuring the column is valid - maybe not useful finally
    ...
    cut_out_col = (row.iloc[:71] != 0)[
        ::-1
    ].idxmax()  # Finding the last column index where value isn't zero
    if (
        cut_out_col in dataframe.columns
    ):  # Ensuring the column is valid - maybe not useful finally
...
bin_edges = np.linspace(
    dataframe["Max Power Output"].min(), dataframe["Max Power Output"].max(), 6
)  # Create 5 equal bins
bin_labels = [
    f"Bin {i+1}" for i in range(len(bin_edges) - 1)
]  # Label bins as Bin 1, Bin 2, etc.

# Assign bins to "Max Power Output"
dataframe["Power Output Bin"] = pd.cut(
    dataframe["Max Power Output"],
    bins=bin_edges,
    labels=bin_labels,
    include_lowest=True,
)
