...
data["Rotor diameter"] = pd.to_numeric(
    data["Rotor diameter"].astype(str).str.replace("m", "", regex=False),
    errors="coerce",
)
data["Rotor weight"] = pd.to_numeric(
    data["Rotor weight"].astype(str).str.replace("Tons", "", regex=False),
    errors="coerce",
)
...
def poly_model(diameter, a, b, c, d):
    return np.maximum(
        a * diameter**3 + b * diameter**2 + c * diameter + d, 0
    )  # Prevents negatives
...
log_params, _ = curve_fit(
    log_model, diameter_offshore, mass_offshore, p0=log_init_guess, maxfev=50000
)
poly_params, _ = curve_fit(
    poly_model, diameter_offshore, mass_offshore, p0=poly_init_guess, maxfev=50000
)
...
# **Calculate RMSE**
rmse_log = np.sqrt(
    np.mean((mass_offshore - log_model(diameter_offshore, *log_params)) ** 2)
)
rmse_poly = np.sqrt(
    np.mean((mass_offshore - poly_model(diameter_offshore, *poly_params)) ** 2)
)
...
print(
    f"Logarithmic Model Coefficients: a={log_params[0]:.4f}, b={log_params[1]:.4f}, c={log_params[2]:.4f}"
)
print(
    f"Polynomial Model Coefficients: a={poly_params[0]:.8f}, b={poly_params[1]:.6f}, c={poly_params[2]:.4f}, d={poly_params[3]:.2f}"
)
...
plt.scatter(
    diameter_offshore, mass_offshore, label="Observed Data", color="blue", alpha=0.6
)
...
plt.plot(
    diameter_range,
    mass_poly_pred,
    label=f"Polynomial Model (RMSE={rmse_poly:.2f})",
    color="green",
)
