...
dataframe["Cut-out Power Output Bin"] = pd.cut(
    dataframe["Max Power Output"],
    bins=bin_edges,
    labels=bin_labels,
    include_lowest=True,
...
dataframe["cut-in wind speed"] = pd.to_numeric(
    dataframe["cut-in wind speed"], errors="coerce"
)
dataframe["cut-out wind speed"] = pd.to_numeric(
    dataframe["cut-out wind speed"], errors="coerce"
)
...
dataframe.boxplot(
    column="cut-out wind speed",
    by="Cut-out Power Output Bin",
    grid=False,
    patch_artist=True,
    ax=axs[1],
    showfliers=True,
